...
enum PrairieControlConnectOrigin {
    case user
    case reconnect
    case autoResume
}
...
private enum PrairieControlHandoffError: LocalizedError {
    ...
    var errorDescription: String? {
        switch self {
        case .updateRequired:
            return "Update Prairie on the TV to play with your profile."
        case .timedOut:
...
@MainActor
@Observable
final class PrairieControlClient {
    private(set) var activeTarget: PrairieControlTarget?
    ...
    private var handoffChallenge: PrairieControlHandoffChallenge?
    private var handoffReady: PrairieControlHandoffReady?
    private var handoffCancellation: PrairieControlHandoffCancel?
    private var missedHeartbeats = 0
    private(set) var lastTarget: PrairieControlTarget?
    ...
    private static let persistedTargetKey = "prairiecontrol.lastTarget"
    ...
    @discardableResult
    func connect(
        to target: PrairieControlTarget,
        ...
        let session = PrairieControlSession(endpoint: target.endpoint)
    ...
    func play(on target: PrairieControlTarget, request: PrairieControlPlaybackRequest) async {
    ...
    func launch(_ request: PrairieControlPlaybackRequest) async {
        ...
        do {
            ...
            try await session.send(.launch(PrairieControlLaunchRequest(serverId: activeServer.id, playback: request)))
            isConnecting = false
        } catch {
    ...
    private func waitForHandoffReady(requestId: String) async throws -> PrairieControlHandoffReady {
        for _ in 0..<600 {
            if let cancellation = handoffCancellation, cancellation.requestId == requestId {
                throw PrairieControlHandoffError.cancelled(cancellation.message ?? "The TV cancelled profile setup.")
            }
    ...
    private func adoptEffectiveTarget(server: ServerEntry) {
        ...
        let effective = PrairieControlTarget(
            id: target.id,
            name: target.name,
    ...
    func send(_ command: PrairieControlCommand) {
    ...
    func attemptAutoResumeIfIdle() {
        ...
        autoResumeTask = Task { @MainActor [weak self] in
            ...
            let browser = PrairieControlBrowser()
            browser.start()
            defer { browser.stop() }

            var match: PrairieControlTarget?
    ...
    private func handle(_ message: PrairieControlMessage, connectionId: UUID) {
        ...
        case .hello(let hello):
            negotiatedVersion = PrairieControlProtocol.negotiatedVersion(with: hello.supportedVersions)
    ...
    private func beginBackgroundRemoteControlGracePeriod() {
        endBackgroundRemoteControlGracePeriod()
        backgroundTask = UIApplication.shared.beginBackgroundTask(withName: "PrairieControlRemote") { [weak self] in
    ...
    private func persistLastTarget(_ target: PrairieControlTarget) {
    ...
    private func updateNowPlaying(for state: PrairieControlPlaybackState) {
        ...
        nowPlaying.update(
            title: state.title.isEmpty ? "Prairie TV" : state.title,
