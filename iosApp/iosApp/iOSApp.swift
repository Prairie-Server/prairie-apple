...
struct PrairieApp: App {
    ...
    init() {
        ...
        #if os(iOS) || os(tvOS)
        // Aether's optional host stream enters Prairie diagnostics only through a
        // media-specific redactor and the existing consent/debug-logging gate.
        // General Apple unified logs are never collected into a Prairie report.
...
/// `.lifecycle` breadcrumbs from `PrairieApp.init` to the first real screen, and
...
enum LaunchTimeline {
    ...
    /// `PrairieApp.init`. Runs before any consent context exists, so this line
