...
struct AppleTVUserContext: Equatable, Sendable {
    ...
    var pairingDescription: String {
        if shouldStorePreferencesForCurrentUser {
            "Prairie keeps a separate profile pairing for this Apple TV user. Switch users in Control Center to view or change the pairing available to another user."
        } else {
            "This Apple TV user context is not set to keep separate app preferences. Prairie uses the profile pairing available to the current app context, and you can change it here at any time."
        }
    }
}
#endif
