...
/// Engine-neutral chapter projection consumed by Prairie's controls.
...
class PlayerViewModel {
    ...
    /// Realtime AI cues stay in Prairie's product layer because Aether 6.34 does
    ...
    private func applySubtitleAppearanceToPlayer() {
        // Prairie's Aether subtitle overlay reads the published appearance
    ...
    /// Live cues are normalized to Prairie source time before the app-owned
    ...
    /// Always pass an explicit dictionary so Prairie's bearer is shared only with
    ...
    private func applySubtitleTrackSelection(_ trackId: Int64?, reason: String) {
        ...
        if let trackId, SubtitleTrackIdSpace.isAILive(trackId) {
            // Synthetic live cues are rendered by Prairie's overlay and must
    ...
    /// a fullscreen native caption box would draw over Prairie's overlay — so the
...
/// mapping before publishing source-time cues to Prairie's overlay.
@MainActor
private final class LiveSubtitleSinkAdapter: LiveSubtitleSink {
    ...
    func feedCue(_ cue: PlaybackRealtimeSubtitleCue) {
        ...
        // Realtime cue timestamps are already absolute Prairie source time.
