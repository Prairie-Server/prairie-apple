...
struct AetherLoadSpec {
    ...
    /// that array — element `i` is the Prairie id of `options.externalSubtitles[i]`,
    /// and `nil` means "this declared track has no stable Prairie id".
    ///
    ...
    /// here that Aether was never asked to register binds a Prairie id to the Aether
    ...
    init(
        ...
        // `StreamRequest` adds the current Prairie bearer to the plan-provided
        ...
        // Declared tracks and their Prairie aliases are appended together so the
