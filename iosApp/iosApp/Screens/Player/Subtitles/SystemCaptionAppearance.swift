...
//  returned value when content does not supply one. Prairie-controlled text
...
//  selected system profile with Prairie's defaults.
//
...
enum SystemCaptionAppearance {
    ...
    /// not provide a value Prairie can map. Split from the mapping so tests can
    ...
    /// Resolve a value for Prairie's explicit "Use Device Settings" mode.
    ///
