...
/// Owns Prairie video's system-media publication and command routing.
///
...
final class AetherVideoNowPlayingCoordinator {
    struct Handlers {
        ...
        var pause: () -> Void
        var isPaused: () -> Bool
        /// Prairie source-media time, not Aether's transport-local player time.
        var currentTime: () -> Double
        /// Accepts a Prairie source-media seek target.
    ...
    func attach(
        ...
        case .session:
            ...
            // system scrubber and its seek commands stay on Prairie's source
