...
enum DiagnosticsUploadError: Error, Equatable {
    ...
    /// large before it reached Prairie — a 413 with no Prairie error envelope.
    /// Distinct from `.tooLarge` (Prairie's own bundle-size verdict): the server
...
actor DiagnosticsAPI {
    ...
    static func mapUploadError(_ error: HTTPError) -> DiagnosticsUploadError {
        ...
        case let code?:
            // A 413 whose error code is not Prairie's own `too_large` (handled
        ...
        case nil:
            // A 413 with no Prairie error envelope means a proxy in front of the
            // server refused the request body before Prairie ever saw it (Prairie's
