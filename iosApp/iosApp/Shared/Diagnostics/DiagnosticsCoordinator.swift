...
actor DiagnosticsCoordinator {
    ...
    func pendingReports(for binding: DiagnosticsBinding) async -> [PendingReport] {
        ...
        if binding.destinationChoice == .hosted {
            ...
            // path to a user's own Prairie server.
            deleting = []
        }
    ...
    /// their local logs are removed; retries need no Prairie account/server data.
    ...
    private func performUpload(report: PendingReport) async -> DiagnosticsUploadDecision {
        ...
        if destination == .hosted, report.state.hostedRemoteShortID != nil {
            ...
            // longer depends on a fresh capability or Prairie consent request.
            // Poll first so a temporary kill switch, storage outage, or Prairie
    ...
    /// subsystem emits from `PrairieApp.init` onward hit that window on every
