...
/// Mirrors Aether's host diagnostics into Prairie's consent-gated log ring.
///
/// Upstream Aether also writes its original messages to Apple unified logging.
/// Prairie deliberately never harvests that store for a diagnostics bundle. This
/// bridge handles only the optional host callback and applies the media
/// privacy boundary before a line can enter Prairie-owned diagnostics.
