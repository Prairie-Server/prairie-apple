...
struct AppleDeviceIdentity: Sendable {
    ...
    /// Product name the server shows in admin Activity (e.g. `Prairie Apple TV`).
    ...
    private static func currentClientName() -> String {
        #if os(tvOS)
        return "Prairie Apple TV"
        #elseif os(macOS)
        return "Prairie Mac"
        #elseif os(iOS)
        return UIDevice.current.userInterfaceIdiom == .pad ? "Prairie iPad" : "Prairie iPhone"
        #else
        return "Prairie"
        #endif
    }
    ...
    ///
    /// `PrairieBuildChannel` is fed by the `PRAIRIE_BUILD_CHANNEL` build setting
    ...
    static var buildChannel: String {
        ...
        return "dev"
        #else
        let raw = Bundle.main.object(forInfoDictionaryKey: "PrairieBuildChannel") as? String
...
extension AppleDeviceIdentity {
    /// The device + client identity headers every Prairie request carries.
    ...
    func applyHeaders(to request: inout URLRequest) {
        let headers: [(String, String)] = [
            ("X-Prairie-Device-Id", id),
            ("X-Prairie-Device-Name", name),
            ("X-Prairie-Device-Platform", platform),
            ("X-Prairie-Client-Family", clientFamily),
            ("X-Prairie-Client", clientName),
            ("X-Prairie-Client-Version", appVersion),
            ("X-Prairie-Client-Build", appBuild),
            ("X-Prairie-Client-Channel", channel)
        ]
