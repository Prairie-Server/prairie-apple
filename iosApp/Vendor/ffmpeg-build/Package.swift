...
//
// Vendored from https://github.com/mpvkit/ffmpeg-build at commit
// b38afaffe8a38c82c3922d36e044a915bf668f27 (tip of main; includes the HLS
// seek fix landed after the 7.1.0 tag). The upstream repository is archived,
// so its manifest can no longer be corrected when a sibling mpvkit build
// repo re-publishes a release asset. That happened on 2026-04-23: the
// Libuavs3d 1.2.1 artifact was re-uploaded with a new checksum, which makes
// remote resolution of the archived manifest fail permanently. The only
// local change versus upstream is the Libuavs3d checksum below, updated to
// match the re-published artifact (verified against the release's
// Libuavs3d.xcframework.checksum.txt and a fresh download).
//
// The directory is named "ffmpeg-build" on purpose: the local package
// identity is derived from it, and scripts/prepare-macos-ffmpeg-frameworks.sh
// looks up binary artifacts under SourcePackages/artifacts/ffmpeg-build.
