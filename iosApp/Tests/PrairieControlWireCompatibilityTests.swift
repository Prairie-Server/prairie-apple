...
/// `PrairieControlProtocol.version` at 2 and kept advertising `[1, 2]`, so a
...
/// `PrairieCastPlaybackState.supportsHDRToggle` is a non-null `Boolean` with no
...
///
/// NOTE: as with `PrairieControlTests`, this project has no unit-test target
/// wired into `project.yml` yet, so these are not currently compiled or
/// executed.
final class PrairieControlWireCompatibilityTests: XCTestCase {
    ...
    func testEncodedStateCarriesSupportsHDRToggleForOldPeers() throws {
        let data = try JSONEncoder().encode(PrairieControlMessage.state(.compatFixture()))
    ...
    func testRetiredSetHDREnabledCommandDecodesAsUnsupportedInsteadOfThrowing() throws {
        ...
        let message = try JSONDecoder().decode(PrairieControlMessage.self, from: data)
    ...
    func testUnsupportedControlRoundTrips() throws {
        let message = PrairieControlMessage.unsupportedControl(name: "set_hdr_enabled")
        let data = try JSONEncoder().encode(message)
        XCTAssertEqual(try JSONDecoder().decode(PrairieControlMessage.self, from: data), message)
    }
    ...
    func testCommandWithoutNameStillFailsToDecode() {
        ...
        XCTAssertThrowsError(try JSONDecoder().decode(PrairieControlMessage.self, from: data))
    }

    // MARK: - Commands: no regression from the hand-written decoder

    /// `PrairieControlCommand` now decodes by hand, so every argument field has
    /// to survive the round trip exactly as the synthesized decoder did.
    func testKnownCommandsRoundTripEveryArgument() throws {
        let commands: [PrairieControlCommand] = [
            .play,
            .pause,
            .playPause,
            .stop,
        ...
        for command in commands {
            let data = try JSONEncoder().encode(PrairieControlMessage.control(command))
            let decoded = try JSONDecoder().decode(PrairieControlMessage.self, from: data)
    ...
    func testCommandWireNamesAreUnchanged() throws {
        let data = try JSONEncoder().encode(PrairieControlMessage.control(.setVideoGravity("fill")))
...
private extension PrairieControlPlaybackState {
    static func compatFixture() -> PrairieControlPlaybackState {
