...
final class ProfileAndQualitySettingsTests: XCTestCase {
    ...
    func testEveryPresetResolutionIsAContractEnumMember() {
        for preset in PrairieQualityPresets.all {
            XCTAssertTrue(
    ...
    func testPresetTableMatchesTheOtherClients() {
        ...
        XCTAssertEqual(PrairieQualityPresets.all.count, expected.count)
        for (actual, want) in zip(PrairieQualityPresets.all, expected) {
    ...
    func testEveryPresetRoundTripsFromItsStoredPair() {
        for preset in PrairieQualityPresets.all {
            let recovered = PrairieQualityPresets.preset(
    ...
    func testPresetsAreDistinguishedByBothAxes() {
        ...
        XCTAssertEqual(PrairieQualityPresets.preset(resolution: "1080p", bitrateKbps: 6_000)?.id, "1080p")
        XCTAssertEqual(PrairieQualityPresets.preset(resolution: "1080p", bitrateKbps: 3_000)?.id, "1080p-low")
    }
    ...
    func testAPairNoPresetCoversHasNoPresetAndDescribesItself() {
        XCTAssertNil(PrairieQualityPresets.preset(resolution: "1080p", bitrateKbps: 8_000))
        XCTAssertEqual(PrairieQualityPresets.describe(resolution: "1080p", bitrateKbps: 8_000), "1080p at 8 Mbps")
        XCTAssertEqual(PrairieQualityPresets.describe(resolution: "720p", bitrateKbps: 1_800), "720p at 1.8 Mbps")
        // A covered pair answers with the preset's own label.
        XCTAssertEqual(PrairieQualityPresets.describe(resolution: "720p", bitrateKbps: 4_000), "720p High")
        XCTAssertEqual(PrairieQualityPresets.describe(resolution: "auto", bitrateKbps: nil), "Auto")
        XCTAssertEqual(PrairieQualityPresets.describe(resolution: "2160p", bitrateKbps: nil), "4K")
    }
    ...
    func testCompoundLegacyResolutionsNormalizeToContractMembers() {
        ...
        XCTAssertEqual(PrairieQualityPresets.normalizeResolution("720p-medium"), "720p")
        XCTAssertEqual(PrairieQualityPresets.normalizeResolution("1080p-8"), "1080p")
        ...
        XCTAssertEqual(PrairieQualityPresets.normalizeResolution("  1080P  "), "1080p")
        // 328p and 420p predate the contract's ladder and have no member.
        XCTAssertEqual(PrairieQualityPresets.normalizeResolution("328p"), "auto")
        ...
        XCTAssertEqual(PrairieQualityPresets.normalizeResolution("1440p"), "auto")

        for resolution in PrairieQualityPresets.all.map(\.resolution) {
            XCTAssertTrue(
                Self.contractResolutions.contains(PrairieQualityPresets.normalizeResolution(resolution)),
    ...
    func testApplyingAPresetWritesBothAxes() async throws {
        ...
        let preset = try XCTUnwrap(PrairieQualityPresets.preset(id: "720p-high"))
    ...
    func testAnUncappedPresetClearsTheBitrateAxis() async throws {
        ...
        harness.settings.setQualityPreset(try XCTUnwrap(PrairieQualityPresets.preset(id: "480p")))
        await harness.settings.flushPendingDeviceSettings()
        harness.transport.reset()

        harness.settings.setQualityPreset(try XCTUnwrap(PrairieQualityPresets.preset(id: "2160p")))
        await harness.settings.flushPendingDeviceSettings()
