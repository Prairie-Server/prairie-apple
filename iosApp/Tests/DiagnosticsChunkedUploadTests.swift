...
final class DiagnosticsChunkedUploadTests: XCTestCase {
    ...
    func testBare413MapsToRequestBlockedByProxy() {
        // An intermediary's 413 carries no Prairie JSON envelope (nginx returns
        // an HTML error page) and must be distinguished from Prairie's own
    ...
    func testJSON413FromNonPrairieProxyStillMapsToRequestBlockedByProxy() {
        // Some proxies emit JSON error envelopes. Any 413 whose code is not
        // Prairie's own `too_large` still came from an intermediary and must
    ...
    func testPrairie413StillMapsToTooLarge() {
    ...
    func testUploadChunkedSplitsSequentiallyAndCompletes() async throws {
        ...
        XCTAssertEqual(response.shortID, "PRAIRIE-TEST12345678")
...
final class ChunkedUploadStubProtocol: URLProtocol {
    ...
    override func startLoading() {
        ...
        case ("POST", "/api/v1/diagnostics/reports/uploads/stub-session/complete"):
            ...
            respond(status: 201, json: #"{"report_id":"11111111-1111-1111-1111-111111111111","short_id":"PRAIRIE-TEST12345678"}"#)
