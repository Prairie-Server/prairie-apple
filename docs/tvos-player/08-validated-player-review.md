...
# Prairie tvOS/iOS Player Validated Review
...
- **PrairiePlayer** currently means `avPlayerLocalDVLoopback`: local FFmpeg remux,
...
2. Surface PrairiePlayer mux/file-write failures through the player error path.
...
| PrairiePlayer is the local Dolby Vision loopback implementation. | Confirmed, narrowed | [`DVSegmentWriter.swift`](../../iosApp/iosApp/Screens/Player/AVPlayerRoute/DVSegmentWriter.swift), [`DVSegmentServer.swift`](../../iosApp/iosApp/Screens/Player/AVPlayerRoute/DVSegmentServer.swift), [`ISOBoxSurgery.swift`](../../iosApp/iosApp/Screens/Player/AVPlayerRoute/ISOBoxSurgery.swift) | Prairie is not a general "all non-native remux" path. H.264 and SDR HEVC loopback are explicitly blocked back toward CompatibilityPlayer in the planner. |
...
| `05-route-capability-matrix.md` says subtitle delay/styling are "Controlled tracks only" for Native/Prairie, while capability code marks those route capabilities `.repoVerified`. | Confirmed, narrowed | Align the table and code wording around the actual scope: Prairie-rendered sidecar/controlled tracks, not all native embedded subtitle behavior. |
...
| `DVSegmentServer.start()` blocks on a semaphore for up to 2 seconds. | Confirmed | Medium | [`DVSegmentServer.swift:106`](../../iosApp/iosApp/Screens/Player/AVPlayerRoute/DVSegmentServer.swift#L106), [`AVPlayerBackend.swift`](../../iosApp/iosApp/Screens/Player/AVPlayerRoute/AVPlayerBackend.swift) | Convert server start to async/throwing startup so Prairie setup does not block the main actor path. |
| Prairie mux write failures are not consistently fatal. | Confirmed, narrowed | High | [`DVSegmentWriter.swift:374, 767, 1654`](../../iosApp/iosApp/Screens/Player/AVPlayerRoute/DVSegmentWriter.swift#L374) | The main loop (`:374`) and pending-audio path (`:767`) log failures from `av_interleaved_write_frame` but continue; only the encoded-audio path (`:1654`) throws. Count/surface the log-only sites and abort through `onFinished(error)` after a small threshold. |
| Prairie file write failures are logged but not surfaced. | Confirmed | High | [`DVSegmentWriter.swift:2212, 2260, 2477, 2503`](../../iosApp/iosApp/Screens/Player/AVPlayerRoute/DVSegmentWriter.swift#L2212) | Init segment (`:2212`), media segment (`:2260`), media playlist (`:2477`), and master playlist (`:2503`) writes catch-and-log without firing `onFinished(error)`. Route them through the error path. |
...
| Three recovery/escalation paths exist. | Confirmed, narrowed ([`PlayerViewModel.swift:1378, 1441, 1795`](../../iosApp/iosApp/Screens/Player/PlayerViewModel.swift#L1378) — native→compat, prairie→compat, VT-H.264→prairie HEVC) | The paths are not all feature-flag gated. They are guarded by active engine, prior-attempt flags, and rejection reason. |
...
| Claim | Status | Notes |
| --- | --- | --- |
| NativePlayer roughly maps to KSPlayer's AVPlayer wrapper layer. | Confirmed | Prairie already has AVPlayer-backed routes. There is little to lift here. |
| PrairiePlayer is not equivalent to a tag-aware AVPlayer wrapper. | Confirmed | Prairie fixes the local source by remuxing/patching/signaling before handing it to AVPlayer. |
...
- Do not treat existing `PlayerCore` PiP plumbing as something that benefits
  NativePlayer/PrairiePlayer without AVPlayer-route PiP work.
...
2. Prairie error propagation: mux write failures, file write failures, localhost
...
selection given fixture sources, and the Prairie mux/file-write error paths once
