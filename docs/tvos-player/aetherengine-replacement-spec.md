...
Prairie Apple will replace its custom library-media playback implementations with
...
PlayerCore, AVPlayerBackend, Prairie's local-HLS/loopback implementation, the
custom source proxy/cache, or the vendored FFmpeg package. Rollback is an older
Prairie build, not an in-process engine switch.
...
Prairie production source constructs no `AVPlayer` directly. The startup treatment
...
2. Preserve Prairie's server session, product, UI, progress, realtime, download,
...
4. Advertise only Protocol V3 capabilities the exact pinned Aether build and
   Prairie adapter have proven on the relevant Apple platform.
...
```text
Prairie controls, settings, queue, progress, realtime, downloads
                              |
...
`AetherEngine.videoRoute` is diagnostic execution state. It is not a Prairie
...
- map a prepared online/offline Prairie plan into an `AetherLoadSpec`;
- project Aether phase, clock, duration, buffer, tracks, chapters, cues, seek
  events, first-frame state, route, telemetry, and typed failures into Prairie;
...
Prairie retains a small pure type that maps among:
...
audio paths. Prairie does not second-guess that decision.
...
refresh API. Prairie's proxy or a permanent custom reader is not restored.
...
- Aether probes the actual source and exact device during `load`, selects its
  native or software path, and a typed load failure enters Prairie's bounded V3
  replan path;
...
and receiver-facing state outside Prairie's redaction boundary. The generic
...
Prairie retains stable server/product identity and maps it to Aether `TrackInfo`
...
Prairie owns a thin UIKit/AppKit/SwiftUI overlay that paints those outputs and
...
by retaining Prairie's old subtitle engine.
...
- AirPlay route UI remains Prairie-owned; Aether owns media-path rewriting and
...
- Video Now Playing binds to Aether's player-scoped session on native routes
  and uses a mutually exclusive shared fallback for software/macOS. Prairie
...
- Audiobooks use a separate Aether audio-only instance and its audio-scoped
  Now Playing session when available. Prairie publishes the stitched whole-book
  timeline rather than per-file transport time.
- Aether owns display criteria. Prairie maps its Match Content preference into
  Aether options and removes its duplicate display-criteria engine code.

## Offline playback

Downloads and their durable metadata remain Prairie-owned. Playback uses Aether
...
- Mirror `EngineLog` only through Prairie's redaction and bounded capture path.
- Upstream Aether's unified-log output is treated as device-local support
  data and is never harvested into a Prairie diagnostics bundle. On iOS and tvOS,
  its optional host callback is mirrored only into Prairie's consent-gated,
...
- Prairie sanitizes all projected media failures before public OSLog output.
  ...
  is not a Prairie distribution blocker under the accepted device-local log
  boundary: Prairie never collects the unified log and masks the separate host
  callback before it enters an uploadable diagnostics artifact.

## Scrub previews

Prairie's controls continue to own preview presentation and request scheduling.
...
- Treat Nuke/NukeUI as shipped package products at their exact resolved
  ...
  separate optional SMB product, but it is not a shipped Prairie component unless
  an installable Prairie target actually links `AetherEngineSMB`.
- Remove Prairie's `Vendor/ffmpeg-build` package and every direct Libav/Libass
...
- Prairie's diagnostics panel projects only public `LiveTelemetry`, route, phase,
...
- The exact resolution lock includes AetherEngine at
  ...
  Aether's optional SMB product but is not linked by Prairie's selected product.
...
- Aether's original local logging-hardening commit passed the 6.34.0 upstream
  ...
  1,955 Swift Testing tests in 282 suites with zero failures. Prairie may retain
...
- A disposable local-package candidate then compiled all three Prairie targets
...
- The live Aether run also reproduced upstream 6.34.0's public source-URL log.
  ...
  custom credential-header leakage. Prairie's diagnostics bridge separately
...
1. No app source or project setting references `PlayerCore`,
   ...
   `AudioPlayerEngine`.
2. No Prairie production source imports `Libav*`, `Libsw*`, `Libass`, or
   `Libdovi` directly.
...
6. An allowlist audit proves that all video, offline video, and audiobook
   playback constructs AetherEngine. Prairie production source contains no direct
...
Rollback: reinstall the previous Prairie build. No runtime engine switch exists.
...
- `server_remux_hls` and `server_transcode_hls` from the real Prairie server;
...
- The app disables subtitle delay/style controls while the selected Aether
  track is natively rendered; fullscreen Aether overlay tracks retain Prairie
