---
title: Prairie Profile Launch Choice and Apple TV User Mapping
...
Prairie keeps the account signed in, retires the old profile identity and profile-scoped work, and asks
...
playback ends while Prairie remains backgrounded. Content deep links stay queued until a new profile is
...
Give people an explicit choice for what happens when a new Prairie app session starts:

1. **Automatic** — use the last successfully selected Prairie profile and go directly to content.
...
Prairie profile, while the household signs in to the Prairie server only once. This uses Apple's current
...
tvOS adds another constraint: multiple Apple TV system users can share one physical device. Prairie
should remember a different app profile for each system user without asking every household member
to authenticate the same Prairie account again.
...
5. On tvOS 16+, remember the Prairie profile separately for each Apple TV system user while sharing
   the Prairie account/server sign-in.
...
- No manual map from deprecated `TVUserIdentifier` values to Prairie profile IDs.
...
- **Ask Every Time** — “Show Who’s Watching when Prairie starts.”

On tvOS, the Automatic helper changes to:

> Use the profile selected for the current Apple TV user.

If the OS cannot retain a current-user preference (for example, an ephemeral user context), Prairie
...
- Settings copy for Automatic should state that anyone able to use this device/Apple TV user can
  open the remembered Prairie profile without entering its PIN again.
...
- the remembered profile is keyed by Prairie `serverId`;
...
| Access and refresh tokens | User-independent Keychain | Avoid a full Prairie login for every Apple TV user |
...
| Launch behavior and remembered profile map | Current Apple TV user | This is the system-user-to-Prairie-profile association |
...
This lets a newly selected Apple TV system user discover the already-signed-in Prairie servers, while
...
Prairie therefore does not need to observe deprecated user-identifier change notifications. Before
...
2. Add two Apple TV system users, map each to a different Prairie profile, and switch A → B → A.
...
7. Switch users while Prairie is foreground/backgrounded and confirm the old profile never flashes.
...
- tvOS system users can share one Prairie account sign-in and keep distinct Prairie profiles.
- Protected profile proof never crosses Apple TV users or Prairie accounts.
