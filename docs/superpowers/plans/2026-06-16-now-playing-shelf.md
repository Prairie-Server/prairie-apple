...
- **Modify** `iosApp/iosApp/Cast/iOS/PrairieCastMiniBar.swift` — accept a `style`, apply chrome via `NowPlayingBarChrome`.
...
> The two `Create` files live under `Cast/iOS/` next to `PrairieCastMiniBar.swift`. They are compiled on all platforms (the cast-specific code inside is `#if os(iOS)`-guarded), so the folder name is cosmetic. After creating a file, run `cd iosApp && xcodegen generate` before building so the generated project picks it up.
...
- Modify: `iosApp/iosApp/Cast/iOS/PrairieCastMiniBar.swift`
...
- [ ] **Step 2: Add `style` to `PrairieCastMiniBar` and route chrome through `NowPlayingBarChrome`**

In `iosApp/iosApp/Cast/iOS/PrairieCastMiniBar.swift`, add the property just under `@State private var artwork`:
...
```bash
cd iosApp && xcodegen generate && xcodebuild build -project Prairie.xcodeproj -scheme Prairie -destination 'platform=iOS Simulator,name=iPhone 17 Pro' CODE_SIGNING_ALLOWED=NO 2>&1 | tail -5
```
...
```bash
git add iosApp/iosApp/Cast/iOS/NowPlayingBarStyle.swift iosApp/iosApp/Cast/iOS/PrairieCastMiniBar.swift iosApp/iosApp/Screens/Audio/AudioMiniPlayerView.swift iosApp/Prairie.xcodeproj
...
struct NowPlayingShelf: View {
    ...
    #if os(iOS)
    @Environment(PrairieCastController.self) private var castController
    #endif
    @Environment(AudioPlaybackStore.self) private var audioStore
    ...
    var body: some View {
        ...
        if castController.hasActiveSession && !castController.isShowingRemoteControl {
            PrairieCastMiniBar(controller: castController, style: style)
...
```bash
git add iosApp/iosApp/Cast/iOS/NowPlayingShelf.swift iosApp/Prairie.xcodeproj
...
```bash
cd iosApp && xcodebuild build -project Prairie.xcodeproj -scheme Prairie -destination 'platform=iOS Simulator,name=iPhone 17 Pro' CODE_SIGNING_ALLOWED=NO 2>&1 | tail -5
```
...
```bash
cd iosApp && xcodebuild build -project Prairie.xcodeproj -scheme PrairieMac -destination 'platform=macOS' CODE_SIGNING_ALLOWED=NO 2>&1 | tail -5
```
...
```bash
cd iosApp && xcodebuild build -project Prairie.xcodeproj -scheme Prairie -destination 'platform=iOS Simulator,name=iPhone 17 Pro' CODE_SIGNING_ALLOWED=NO 2>&1 | tail -3
xcrun simctl boot "iPhone 17 Pro" 2>/dev/null; open -a Simulator
```

Install/launch the freshly built `.app` (path printed in the build log under `Products/Debug-iphonesimulator/Prairie.app`):

```bash
xcrun simctl install booted "$(cd iosApp && xcodebuild -project Prairie.xcodeproj -scheme Prairie -destination 'platform=iOS Simulator,name=iPhone 17 Pro' -showBuildSettings 2>/dev/null | awk '/ BUILT_PRODUCTS_DIR =/{d=$3}/ FULL_PRODUCT_NAME =/{p=$3}END{print d"/"p}')"
xcrun simctl launch booted com.prairie.app 2>/dev/null || xcrun simctl launch booted "$(/usr/libexec/PlistBuddy -c 'Print :CFBundleIdentifier' iosApp/Info.plist 2>/dev/null)"
```

> If the bundle id lookup is unclear, find it with `grep -i bundleIdentifier iosApp/project.yml` or read it from the build settings (`PRODUCT_BUNDLE_IDENTIFIER`). Sign-in for the dev server is admin / water1234 (see the PrairieTV simulator-debugging memory).
...
- If a cast target is available (real Apple TV or a second Prairie TV simulator per the companion-pairing memory), start a cast session and confirm the cast bar appears above the tab bar and that tapping it opens the full remote.
