...
{
  "projectPath": "/Users/<mac-user>/silo-apple-deploy/iosApp/Prairie.xcodeproj",
  "scheme": "Prairie",
...
entitlements. Preserve that local signature for authenticated runs: Prairie's tokens live in the
...
ssh mac-builder 'xcrun devicectl device install app --device <DEVICE_ID> \
  ~/silo-build-ios/Build/Products/Debug-iphoneos/Prairie.app'
...
- **Normally signed simulator builds** — unaffected. Xcode's "Sign to Run Locally" path does not
  need the developer login keychain and preserves Prairie's Keychain-backed session.
...
| `~/silo-apple-deploy/iosApp/Prairie.xcodeproj` | Generated project |
