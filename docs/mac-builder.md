...
Most Prairie development happens on a Linux workstation. Xcode does not run there, so the Apple
...
─────────────────                                   ────────────────
source of truth  ──── rsync / git apply ──────────►  ~/prairie-apple-deploy
...
The Mac holds a **deploy checkout** at `~/prairie-apple-deploy` that mirrors the Linux working tree
...
rsync -az --delete \
  ...
  /path/to/prairie-apple/ mac-builder:~/prairie-apple-deploy/

ssh mac-builder 'cd ~/prairie-apple-deploy && git log --oneline -1 && git status --short | wc -l'
```
...
```bash
git diff <base-commit> > /tmp/prairie.patch
ssh mac-builder 'cd ~/prairie-apple-deploy && git checkout -- .'
ssh mac-builder 'cd ~/prairie-apple-deploy && git apply - && git status --short' < /tmp/prairie.patch
```
...
**Then regenerate the project.** `Prairie.xcodeproj` is generated by XcodeGen and is not committed:

```bash
ssh mac-builder 'cd ~/prairie-apple-deploy/iosApp && /opt/homebrew/bin/xcodegen generate'
```
...
{
  "projectPath": "/Users/<mac-user>/prairie-apple-deploy/iosApp/Prairie.xcodeproj",
  "scheme": "Prairie",
...
Schemes: `Prairie` (iOS), `PrairieTV` (tvOS), `PrairieMac` (macOS).
...
| tvOS | same tools, `{"scheme": "PrairieTV", "simulatorId": "<tvos-sim>"}` |
...
```bash
ssh mac-builder 'cd ~/prairie-apple-deploy/iosApp && \
  xcodebuild build -project Prairie.xcodeproj -scheme PrairieTV \
  -destination "generic/platform=tvOS" CODE_SIGNING_ALLOWED=NO 2>&1 \
  | grep -E "error:|BUILD (SUCCEEDED|FAILED)"'
```

Swap `-scheme PrairieMac -destination "platform=macOS"` for the Mac app.
...
ssh mac-builder 'security unlock-keychain -p "$KEYCHAIN_PASSWORD" ~/Library/Keychains/login.keychain-db && \
  cd ~/prairie-apple-deploy/iosApp && \
  xcodebuild build -project Prairie.xcodeproj -scheme Prairie \
    -destination "id=<DEVICE_ID>" \
    -derivedDataPath ~/prairie-build-ios \
...
ssh mac-builder 'xcrun devicectl device install app --device <DEVICE_ID> \
  ~/prairie-build-ios/Build/Products/Debug-iphoneos/Prairie.app'

ssh mac-builder 'xcrun devicectl device process launch --device <DEVICE_ID> org.prairieserver.prairie'
```

Stable derived-data paths (`~/prairie-build-ios`, `~/prairie-build-tvos`) keep incremental builds warm
...
ssh mac-builder 'nohup xcrun devicectl device process launch \
  --device <DEVICE_ID> --console org.prairieserver.prairie \
  >> /tmp/prairie-run.log 2>&1 </dev/null & disown'
```
...
- **Simulator versus device destinations produce different product directories**
  ...
  simulator bundle.
- **Run `xcodegen generate` after syncing.** `Prairie.xcodeproj` is generated; a stale one omits new
...
| Path / value | What |
|---|---|
| `~/prairie-apple-deploy` | Deploy checkout on the Mac (build scratch, never edit) |
| `~/prairie-apple-deploy/iosApp/Prairie.xcodeproj` | Generated project |
| `~/prairie-build-ios`, `~/prairie-build-tvos` | Stable derived-data paths |
...
| `Prairie` / `PrairieTV` / `PrairieMac` | iOS / tvOS / macOS schemes |
| `org.prairieserver.prairie` | Default bundle ID (override in `Local.xcconfig`) |

## Extending this to another Mac or another project

Nothing here is Prairie-specific except the paths and scheme names. To point the same setup at a
