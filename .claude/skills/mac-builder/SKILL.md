...
The Mac has its own checkout at `~/prairie-apple-deploy`. It is build scratch. **Never edit source
...
rsync -az --delete --exclude 'DerivedData' --exclude 'build' \
  /srv/dev/github/Prairie-Server/prairie-apple/ mac-builder:~/prairie-apple-deploy/
```
...
`Prairie.xcodeproj` is generated by XcodeGen and is not committed. A stale one silently omits new
...
| Unit tests | `test_sim` |
| tvOS / macOS | same tools with `{"scheme": "PrairieTV"}` / `build_macos`, `test_macos` |
...
`-scheme PrairieMac -destination "platform=macOS"` for the Mac app.
...
Use the stable derived-data paths `~/prairie-build-ios` / `~/prairie-build-tvos` so incremental builds
...
ssh mac-builder 'security unlock-keychain -p "$KEYCHAIN_PASSWORD" ~/Library/Keychains/login.keychain-db && \
  ...
  xcodebuild build -project Prairie.xcodeproj -scheme Prairie \
    -destination "id=<DEVICE_ID>" -derivedDataPath ~/prairie-build-ios \
...
- Commits, pushes, and PRs happen on Linux against the real working tree — never from
  `~/prairie-apple-deploy`.
- Codex memory may reference a helper at `/home/dev/.local/bin/prairie-apple-deploy`. **That script
  no longer exists.** Use the sync steps above.

## Not for this skill

- Editing Swift source — do that in the local checkout, then sync.
- Prairie server or Android work.
